...
import { ArrowLeft, Store, Shield, Sparkles, Check } from 'lucide-react';

export const metadata: Metadata = {
  title: 'Sign In | SHOP.CO',
  description: 'Sign in to your SHOP.CO customer account or continue as provider.',
};
    ...
    <div className="min-h-screen bg-[#FAFAFA] flex flex-col justify-between text-black">
      ...
      <header className="border-b border-gray-100 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4 text-sm">
            ...
            {/* Direct SSR Link for Provider Login */}
            {!isProviderMode ? (
              <Link
      ...
      <main className="flex-1 w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 flex items-center justify-center">
        <div className="w-full max-w-5xl grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
          ...
          <div className="hidden lg:flex lg:col-span-5 flex-col justify-between p-7 rounded-3xl bg-black text-white relative overflow-hidden min-h-[480px] shadow-xl">
            ...
            <div className="relative z-10">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/10 text-xs font-semibold tracking-wide uppercase text-white/90">
                ...
                {isProviderMode ? 'Merchant Network' : 'Exclusive Style'}
              </span>
              ...
              <p className="text-xs sm:text-sm text-gray-400 mt-3 leading-relaxed">
                {isProviderMode
                  ? 'Access your unified seller dashboard to track daily sales, restock collections, and process customer fulfillment effortlessly.'
                  : 'Sign in to access your curated wardrobe, save your favorite outfits, and enjoy seamless fast-track checkout.'}
              </p>
            </div>
            ...
            <div className="relative z-10 space-y-2.5 my-6">
              <div className="flex items-center gap-2.5 text-xs sm:text-sm text-gray-300">
                ...
                <span>{isProviderMode ? 'Real-time inventory & sales insights' : '200+ International fashion brands'}</span>
              </div>
              <div className="flex items-center gap-2.5 text-xs sm:text-sm text-gray-300">
                <div className="w-5 h-5 rounded-full bg-white/10 flex items-center justify-center text-white shrink-0">
                  <Check size={12} />
                </div>
                <span>{isProviderMode ? 'Automated payout & order fulfillment' : 'Over 30,000+ satisfied customers'}</span>
              </div>
              <div className="flex items-center gap-2.5 text-xs sm:text-sm text-gray-300">
                <div className="w-5 h-5 rounded-full bg-white/10 flex items-center justify-center text-white shrink-0">
                  <Check size={12} />
                </div>
                <span>{isProviderMode ? 'Dedicated seller support team' : '100% genuine guaranteed products'}</span>
              </div>
            </div>
          ...
          <div className="lg:col-span-7 flex justify-center">
            <div className="w-full max-w-md bg-white p-5 sm:p-7 rounded-3xl border border-gray-200/80 shadow-xs">
              
              {/* Header Titles */}
              <div className="mb-4">
                <h1 className="text-xl sm:text-2xl font-black tracking-tight text-gray-950">
                  {isProviderMode ? 'Provider Sign In' : 'Welcome Back'}
                </h1>
                <p className="text-xs sm:text-sm text-gray-500 mt-1">
                  {isProviderMode
                    ? 'Enter your merchant credentials to manage your store.'
      ...
      <footer className="border-t border-gray-100 bg-white py-6">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-gray-500">
          ...
          <div className="flex items-center gap-6">
            <Link href="/login?role=provider" className="hover:text-black font-semibold transition-colors">
              Continue as provider
            </Link>
            <Link href="/register" className="hover:text-black transition-colors">
              Customer Registration
            </Link>
            <Link href="/register?role=provider" className="hover:text-black transition-colors">
              Become a provider
            </Link>
          </div>
        </div>
